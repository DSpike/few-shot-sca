"""
Simplified Stable Few-Shot SCA
===============================
More conservative approach to avoid hanging
"""

import h5py
import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
import torch.optim as optim
from collections import defaultdict
import sys

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Device: {device}\n")

# Reduced parameters for stability
K_SHOT = 10
N_FEATURES = 500  # Fewer features
META_ITERS = 50   # Fewer iterations to test
INNER_STEPS = 5   # Fewer adaptation steps
INNER_LR = 0.01
META_LR = 0.001

sbox = np.array([
    
], dtype=np.uint8)

print("Loading RAW ASCAD traces...")
sys.stdout.flush()

file_path = r'C:\Users\Dspike\Documents\sca\ASCAD_data\ATMEGA_AES_v1\ATM_AES_v1_fixed_key\ASCAD_data\ASCAD_databases\ATMega8515_raw_traces.h5'

with h5py.File(file_path, 'r') as f:
    # Load subset for faster processing
    X_prof = np.array(f['traces'][:30000], dtype=np.float32)  # Reduced from 50k
    X_attack = np.array(f['traces'][30000:40000], dtype=np.float32)  # 10k attack traces

    meta_prof = f['metadata'][:30000]
    meta_attack = f['metadata'][30000:40000]

    # Byte 3
    plaintext_prof = np.array([m[0][3] for m in meta_prof])
    key_byte = meta_prof[0][2][3]
    masks_prof = np.array([m[3][3] for m in meta_prof])
    y_prof = sbox[plaintext_prof ^ key_byte ^ masks_prof]

    plaintext_attack = np.array([m[0][3] for m in meta_attack])
    masks_attack = np.array([m[3][3] for m in meta_attack])
    correct_key = meta_attack[0][2][3]

print(f"✓ Loaded {len(X_prof)} profiling, {len(X_attack)} attack traces")
print(f"✓ Correct key: {hex(correct_key)}")
sys.stdout.flush()

# Fast SNR computation (sample 1000 points only)
print("\nComputing SNR (fast sampling)...")
sys.stdout.flush()

sample_indices = np.linspace(0, X_prof.shape[1]-1, 1000, dtype=int)
snr_values = np.zeros(1000)

for idx, i in enumerate(sample_indices):
    if idx % 100 == 0:
        print(f"  Progress: {idx}/1000")
        sys.stdout.flush()

    class_means = []
    class_vars = []
    for c in range(256):
        mask = (y_prof == c)
        if mask.sum() > 2:
            class_means.append(np.mean(X_prof[mask, i]))
            class_vars.append(np.var(X_prof[mask, i]))

    if len(class_means) > 1:
        snr_values[idx] = np.var(class_means) / (np.mean(class_vars) + 1e-12)

print(f"\nSNR stats: min={snr_values.min():.4f}, max={snr_values.max():.4f}")
sys.stdout.flush()

# Select top features
top_idx = np.argsort(snr_values)[-N_FEATURES:]
selected_points = sample_indices[top_idx]
selected_points = np.sort(selected_points)

X_prof_sel = X_prof[:, selected_points]
X_attack_sel = X_attack[:, selected_points]

# Normalize
mean = X_prof_sel.mean(axis=0)
std = X_prof_sel.std(axis=0) + 1e-8
X_prof_sel = (X_prof_sel - mean) / std
X_attack_sel = (X_attack_sel - mean) / std

# Split
split = int(0.8 * len(X_prof_sel))
X_train, y_train = X_prof_sel[:split], y_prof[:split]
X_test, y_test = X_prof_sel[split:], y_prof[split:]

print(f"✓ Train: {len(X_train)}, Test: {len(X_test)}\n")
sys.stdout.flush()

# Simpler model
class SimpleCNN(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv1d(1, 32, 11, padding=5)
        self.bn1 = nn.BatchNorm1d(32)
        self.conv2 = nn.Conv1d(32, 64, 11, padding=5)
        self.bn2 = nn.BatchNorm1d(64)
        self.pool = nn.AdaptiveAvgPool1d(1)
        self.fc = nn.Linear(64, 256)

    def forward(self, x):
        if x.dim() == 2:
            x = x.unsqueeze(1)
        x = F.relu(self.bn1(self.conv1(x)))
        x = F.relu(self.bn2(self.conv2(x)))
        x = self.pool(x).squeeze(-1)
        return self.fc(x)

class TaskSampler:
    def __init__(self, X, y, k_shot=10):
        self.X = X
        self.y = y
        self.k_shot = k_shot
        self.class_idx = defaultdict(list)
        for i, label in enumerate(y):
            self.class_idx[label].append(i)
        self.valid_classes = [c for c in range(256) if len(self.class_idx[c]) >= k_shot + 3]
        print(f"  Valid classes: {len(self.valid_classes)}/256")
        sys.stdout.flush()

    def sample(self):
        sup_x, sup_y, qry_x, qry_y = [], [], [], []
        for cls in self.valid_classes:
            idx = self.class_idx[cls]
            sel = np.random.choice(idx, self.k_shot + 3, replace=False)
            sup_x.append(self.X[sel[:self.k_shot]])
            sup_y.append([cls] * self.k_shot)
            qry_x.append(self.X[sel[self.k_shot:]])
            qry_y.append([cls] * 3)
        return (torch.FloatTensor(np.vstack(sup_x)), torch.LongTensor(np.hstack(sup_y)),
                torch.FloatTensor(np.vstack(qry_x)), torch.LongTensor(np.hstack(qry_y)))

def maml_inner_loop(model, sup_x, sup_y):
    adapted = SimpleCNN().to(device)
    adapted.load_state_dict(model.state_dict())
    optimizer = optim.SGD(adapted.parameters(), lr=INNER_LR)
    for _ in range(INNER_STEPS):
        optimizer.zero_grad()
        loss = F.cross_entropy(adapted(sup_x), sup_y)
        loss.backward()
        optimizer.step()
    return adapted

print("="*70)
print(f"Training MAML ({K_SHOT}-shot, {META_ITERS} iterations)")
print("="*70)
sys.stdout.flush()

sampler = TaskSampler(X_train, y_train, k_shot=K_SHOT)
model = SimpleCNN().to(device)
meta_optimizer = optim.Adam(model.parameters(), lr=META_LR)

print("Starting training loop...")
sys.stdout.flush()

for i in range(META_ITERS):
    meta_optimizer.zero_grad()
    total_loss = 0

    # Single task per iteration (simpler)
    sup_x, sup_y, qry_x, qry_y = sampler.sample()
    sup_x, sup_y = sup_x.to(device), sup_y.to(device)
    qry_x, qry_y = qry_x.to(device), qry_y.to(device)

    adapted = maml_inner_loop(model, sup_x, sup_y)
    loss = F.cross_entropy(adapted(qry_x), qry_y)
    loss.backward()
    meta_optimizer.step()

    if (i + 1) % 10 == 0:
        print(f"  Iter {i+1}/{META_ITERS}: Loss = {loss.item():.4f}")
        sys.stdout.flush()

print("\n✓ Training complete")
sys.stdout.flush()

# Attack
print("\nRunning attack...")
sys.stdout.flush()

test_sampler = TaskSampler(X_test, y_test, k_shot=K_SHOT)
sup_x, sup_y, _, _ = test_sampler.sample()
adapted_model = maml_inner_loop(model, sup_x.to(device), sup_y.to(device))
adapted_model.eval()

all_probs = []
batch_size = 100
for i in range(0, len(X_attack_sel), batch_size):
    batch = torch.FloatTensor(X_attack_sel[i:i+batch_size]).to(device)
    with torch.no_grad():
        logits = adapted_model(batch)
        probs = F.softmax(logits, dim=1).cpu().numpy()
    all_probs.append(probs)
    del batch, logits
    torch.cuda.empty_cache()

probs = np.vstack(all_probs)
log_probs = np.log(probs + 1e-40)

print("\nKey Recovery Results:")
for n in [100, 500, 1000, 5000, 10000]:
    if n > len(log_probs):
        break
    scores = np.zeros(256)
    for k in range(256):
        intermediate = sbox[plaintext_attack[:n] ^ k ^ masks_attack[:n]]
        scores[k] = np.sum(log_probs[np.arange(n), intermediate])
    rank = np.argsort(-scores).tolist().index(correct_key)
    status = "✓ SUCCESS!" if rank == 0 else ("✓ GOOD!" if rank < 20 else "")
    print(f"  {n:5d} traces → rank {rank:3d}  {status}")

print("\n✓ Complete!")
