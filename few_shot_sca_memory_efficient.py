"""
Memory-Efficient Few-Shot SCA
==============================
Processes attack traces in batches to avoid GPU memory issues
"""
...
INNER_LR = 0.01
META_LR = 0.001
ATTACK_BATCH_SIZE = 100  # Process attack traces in batches
...
variances = np.var(X_prof, axis=0)
top_idx = np.argsort(variances)[-N_FEATURES:]
top_idx = np.sort(top_idx)

X_prof = X_prof[:, top_idx]
X_attack = X_attack[:, top_idx]
...
# Task sampler
class TaskSampler:
    def __init__(self, X, y, k_shot=5):
        self.X = X
        self.y = y
        self.k_shot = k_shot
...
# MAML
def maml_inner_loop(model, sup_x, sup_y, steps=5, lr=0.01):
...
plt.tight_layout()
plt.savefig('maml_training.png', dpi=100)
print("\n✓ Saved: maml_training.png")

# Evaluate - MEMORY EFFICIENT VERSION
print("\n" + "="*60)
print("Attack Evaluation (Memory-Efficient)")
print("="*60)
...
# Process attack traces in batches to avoid memory issues
print("Running attack (processing in batches)...")
adapted_model.eval()
...
with torch.no_grad():
    for batch_idx in range(n_batches):
        ...
        if (batch_idx + 1) % 10 == 0:
            print(f"  Processed {end_idx}/{len(X_attack)} traces...")

probs = np.vstack(all_probs)
log_probs = np.log(probs + 1e-40)

print("Computing key ranks...")
...
plt.ylabel('Key rank')
plt.title(f'MAML Attack Results ({K_SHOT}-shot)')
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('attack_results.png', dpi=100)
print("\n✓ Saved: attack_results.png")
...
print("\n✓ Complete!")
print("\nNext steps:")
print("  1. If results are good, increase META_ITERS to 500-1000")
print("  2. Try different K_SHOT values (2, 10, 20)")
print("  3. Compare with baseline (train CNN on k-shot data only)")
print("  4. Read RESEARCH_GUIDE.md for full experiments")
